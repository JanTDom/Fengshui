        ...
        <div className="report-modal-header">
          <div className="report-modal-title">
            <div className="report-badge">
              <Sparkles size={16} />
              <span>Praktyczny Audyt Przestrzenny & Feng Shui</span>
            </div>
            <h2>Twój Raport Przestrzenny — Jasne Wnioski Krok po Kroku</h2>
          </div>
        ...
        <div className="report-modal-body">
          ...
          <div className="report-cover-box">
            <div className="report-score-block">
              <span className="score-label">Ocena potencjału układu</span>
              <div className="score-number-row">
                <strong>{report.score}</strong>
                <span>/100</span>
              </div>
              <small>Poziom pewności danych: {confidenceLabel(report.confidence)}</small>
            </div>

            <div className="report-decision-block">
              <span className="decision-label">Werdykt w pigułce: Czy to dobry układ?</span>
          ...
          <div className="report-summary-box">
            <h3>Podsumowanie dla Ciebie w 30 sekund</h3>
                ...
                {report.resident_analysis.map((res, idx) => (
                  <article key={idx} style={{ background: "#FFFFFF", border: "1px solid var(--line)", borderRadius: "10px", padding: "14px 16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                      <strong style={{ fontSize: "0.95rem", color: "var(--ink)" }}>{res.name} ({res.role || "Domownik"})</strong>
                      <span style={{ background: "rgba(197, 150, 66, 0.15)", color: "#C59642", fontWeight: 800, fontSize: "0.72rem", padding: "3px 8px", borderRadius: "99px" }}>
                        {res.kua_number ? `Profil Kua ${res.kua_number} (${res.element})` : res.group}
                      </span>
                    </div>
                    ...
                    {res.favorable_directions?.length ? (
                      <div style={{ fontSize: "0.75rem", color: "#2E7D5A", fontWeight: 600, background: "rgba(46, 125, 90, 0.08)", padding: "6px 8px", borderRadius: "6px" }}>
                        ✨ Twoje najlepsze kierunki: {res.favorable_directions.join(" · ")}
                      </div>
                    ) : null}
                  </article>
                ))}
          ...
          {report.tiered_recommendations ? (
            <div style={{ marginTop: "24px", marginBottom: "20px" }}>
              <h3 style={{ fontSize: "1.05rem", fontWeight: 800, color: "var(--ink)", marginBottom: "12px" }}>Plan działania — od najprostszych zmian bez remontu</h3>
          ...
          {report.prioritized_issues && report.prioritized_issues.length > 0 ? (
            <div style={{ marginTop: "24px", marginBottom: "20px" }}>
              <h3 style={{ fontSize: "1.05rem", fontWeight: 800, color: "var(--ink)", marginBottom: "12px" }}>Najważniejsze korekty uszeregowane priorytetem (P1–P4)</h3>
                ...
                {report.prioritized_issues.map((iss, idx) => (
                  <div key={idx} style={{ display: "flex", alignItems: "flex-start", gap: "12px", background: "#FBF9F4", border: "1px solid #E0D7C6", borderRadius: "8px", padding: "10px 14px" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <small style={{ fontSize: "0.75rem", color: "#166534", fontWeight: 700 }}>Co dokładnie zrobić: {iss.remedy_action}</small>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <article key={idx} className="report-room-card">
                    ...
                    {room.recommendations && room.recommendations.length > 0 ? (
                      <div className="room-remedy-box">
                        <strong>Konkretne porady:</strong>
          ...
          {report.sector_map && report.sector_map.length > 0 ? (
            <div className="report-bagua-sectors-section" style={{ marginTop: "24px", marginBottom: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <h3 style={{ fontSize: "1.05rem", fontWeight: 800, color: "var(--ink)", margin: 0 }}>Siatka 9 stref domu (Sektory Bagua)</h3>
                <span style={{ fontSize: "0.75rem", background: "rgba(45, 90, 70, 0.1)", color: "#2D5A46", padding: "4px 8px", borderRadius: "6px", fontWeight: 700 }}>9 stref życiowych</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "10px" }}>
                {report.sector_map.map((sec, idx) => (
                  <div key={idx} style={{ background: "#FBF9F4", border: "1px solid var(--line)", borderRadius: "8px", padding: "10px 12px", display: "flex", flexDirection: "column", gap: "4px" }}>
                    ...
                    <small style={{ fontSize: "0.74rem", color: "#2E7D5A", fontWeight: 600 }}>Funkcja w Twoim domu: {sec.current_use}</small>
